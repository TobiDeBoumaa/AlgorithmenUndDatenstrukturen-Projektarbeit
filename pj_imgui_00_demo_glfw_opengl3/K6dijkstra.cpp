...
void dijkstra::print_path(int i) {
...
void dijkstra::print_path_mac(int i) {
  if (n_info[i].predecessor != -1)
    print_path(n_info[i].predecessor);
  printf("↳ %s MAC:(%s)\n", knoten[i].name.c_str(),
...
void dijkstra::print_path(mac eingabeAddresse, int displayoption) {
  int i = 0;
  for (i = 0; i < ANZAHL; i++) {
...
void dijkstra::print_all() {
  ...
  for (i = 0; i < ANZAHL; i++) {
    vector<Knoten> liste;
    get_path(i, liste);
    printf("%s\t%s\t", liste.back().name.c_str(),
           liste.back().macAddress.macString.c_str());
    if (liste.size() > 1) {
      for (vector<Knoten>::reverse_iterator it = liste.rbegin();
           it != liste.rend() - 1; it++)
        printf("%s->", it->name.c_str());
      printf("%s", liste.front().name.c_str());
    } else
      printf("-");
    printf("\n");
  }
...
Knoten dijkstra::macToKnoten(mac eingabeAddresse) {
  int i = 0;
  for (i = 0; i < ANZAHL; i++) {
    if (find_path(i).macAddress == eingabeAddresse)
      break;
  }
  return find_path(i);
}
